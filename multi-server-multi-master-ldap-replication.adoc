...
When `zmldapenable-mmr` is run the first time, the master LDAP servers
...
Note that once MMR is enabled on an LDAP server, that LDAP server will also support LDAP hosts configured as traditional LDAP Replica servers.
